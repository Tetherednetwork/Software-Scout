...
import { CloseIcon, DownloadIcon, HistoryIcon, SoftwareIcon, WrenchIcon, CheckCircleIcon, CloseCircleIcon } from './Icons';
...
const DownloadHistoryModal: React.FC<DownloadHistoryModalProps> = ({ history, onClose, onUpdateStatus }) => {
            ...
            <div className="w-full max-w-2xl bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 text-left relative flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">A record of all software and drivers you've downloaded.</p>
                    </div>
                ...
                <div className="p-2 sm:p-4 overflow-y-auto">
                    {history.length > 0 ? (
                                ...
                                <li key={item.id} className="p-4 flex items-center justify-between gap-4 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    ...
                                    {item.url ? (
                                        <button 
                                            ...
                                            aria-label={`Download ${item.name} again`}
                                            className="flex-shrink-0 inline-flex items-center justify-center gap-2 px-4 py-2 bg-green-500 text-white text-sm font-semibold rounded-md hover:bg-green-600 transition-colors disabled:bg-gray-400 disabled:cursor-wait w-40"
                                        >
                                            {checkingId === item.id ? (
                                        ...
                                        <span
                                            ...
                                            className="flex-shrink-0 inline-flex items-center justify-center gap-2 px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-600 dark:text-gray-400 text-sm font-semibold rounded-md cursor-not-allowed w-40"
                                        >
                                            <DownloadIcon />
                        ...
                        <div className="text-center py-16 px-4">
                            ...
                            <h3 className="mt-4 text-lg font-semibold text-gray-800 dark:text-gray-200">No downloads yet</h3>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">When you download software or a driver, it will appear here.</p>
                        </div>
                    )}
                </div>
