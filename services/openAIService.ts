...
// This system instruction is adapted from the Gemini service but modified for OpenAI's API
// and its lack of a built-in search tool. It instructs the model to find and embed URLs directly.
const openAISystemInstruction = `You are SoftMonk, an AI cybersecurity assistant. Your single most important mission is to protect users by providing safe, verified, direct download links from official sources ONLY. User safety is your absolute priority. Failure to adhere to these rules can put users at risk, so you must be strict.
...
5.  **Final Step: Search and Respond**: Once you have all information (manufacturer, model, OS, component), use your web search to find the SINGLE official OEM driver download page.
...
export const findSoftware = async (history: Message[], filter: SoftwareFilter, session: Session | null): Promise<BotResponse> => {
    ...
    try {
        ...
        // This part makes the OpenAI response compatible with the existing UI components.
        let groundingChunks: GroundingChunk[] | undefined = undefined;
        let textForDisplay = rawText;

        // Regex to find any markdown-style URL for the official source or guide.
        ...
        if (sourceMatch && sourceMatch[1]) {
            ...
            // Create a fake grounding chunk to be used by the MessageItem component.
            groundingChunks = [{ web: { uri: url, title: 'Official Source' } }];
            // Remove the source line from the text to avoid showing the URL twice.
