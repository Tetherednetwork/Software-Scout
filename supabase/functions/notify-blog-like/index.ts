...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (like.post_id) {
      ...
      if (!post) return new Response('ok'); // Post might have been deleted

      recipientId = post.user_id;
      notificationContent = `${likerUsername} liked your blog post: "${post.title}"`;
      linkUrl = `/blog-post/${like.post_id}`;
      ...
      if (commentError) throw new Error(`Could not fetch blog comment: ${commentError.message}`);
      if (!comment) return new Response('ok'); // Comment might have been deleted
      ...
      if (postError) throw new Error(`Could not fetch blog post for comment: ${postError.message}`);

      recipientId = comment.user_id;
      notificationContent = `${likerUsername} liked your comment on "${post?.title || 'a blog post'}"`;
      linkUrl = `/blog-post/${comment.post_id}`;
      relatedEntityId = like.comment_id;
    }

    // Prevent self-notification and ensure we have a recipient
